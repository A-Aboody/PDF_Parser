...
class TestFollowUpQuestions(unittest.TestCase):
    ...
    def test_follow_up_questions(self, mock_chain, mock_memory, mock_hub, 
        ...
        first_question = "What is the main topic of this paper?"
        first_answer = "The paper is about machine learning for climate change prediction."
        ...
        with patch('time.time') as mock_time:
            ...
            handle_userinput(first_question)
        
        self.assertEqual(len(st.session_state.chat_history), 1)
        self.assertEqual(st.session_state.chat_history[0].content, first_answer)
        
        follow_up_question = "What methods did they use?"
        follow_up_answer = "They used a combination of convolutional neural networks (CNNs) and recurrent neural networks (RNNs)."
        ...
        with patch('time.time') as mock_time:
            ...
            handle_userinput(follow_up_question)
        
        self.assertEqual(len(st.session_state.chat_history), 2)
        self.assertEqual(st.session_state.chat_history[1].content, follow_up_answer)
        
        self.assertGreaterEqual(mock_chain_instance.call_count, 2)
        last_call_args = mock_chain_instance.call_args[0][0]
        self.assertEqual(last_call_args['question'], follow_up_question)
        
        self.assertEqual(len(st.session_state.metrics["questions"]), 2)
        self.assertEqual(st.session_state.metrics["questions"][0], first_question)
        self.assertEqual(st.session_state.metrics["questions"][1], follow_up_question)
        
        self.assertEqual(len(st.session_state.metrics["response_times"]), 2)
        self.assertEqual(len(st.session_state.metrics["interactions"]), 2)
        self.assertEqual(st.session_state.metrics["interactions"][1], 2)
